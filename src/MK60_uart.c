...
void uart_init (UARTn_e uartn, uint32 baud)
{
    ...
    //写 SBR
    temp = UART_BDH_REG(UARTN[uartn]) & (~UART_BDH_SBR_MASK);           //缓存 清空 SBR 的 UARTx_BDH的值
    UART_BDH_REG(UARTN[uartn]) = temp |  UART_BDH_SBR(sbr >> 8);        //先写入SBR高位
    UART_BDL_REG(UARTN[uartn]) = UART_BDL_SBR(sbr);                     //再写入SBR低位

    //写 BRFD
    temp = UART_C4_REG(UARTN[uartn]) & (~UART_C4_BRFA_MASK) ;           //缓存 清空 BRFA 的 UARTx_C4 的值
    UART_C4_REG(UARTN[uartn]) = temp |  UART_C4_BRFA(brfa);             //写入BRFA



    //设置FIFO(FIFO的深度是由硬件决定的，软件不能设置)
    UART_PFIFO_REG(UARTN[uartn]) |= (0
...
void uart_getchar (UARTn_e uartn, char *ch)
{
    ...
    //while( !UART_RCFIFO_REG(UARTN[uartn]) );                                //等待接收到数据数量非0
    //while( (UART_SFIFO_REG(UARTN[uartn]) & UART_SFIFO_RXEMPT_MASK)) ;     //等待接收缓冲区/FIFO 非空的

    //如下方法是飞思卡尔官方例程提供的方法
    while (!(UART_S1_REG(UARTN[uartn]) & UART_S1_RDRF_MASK));       //等待接收满了
 ...
 */
char uart_querychar (UARTn_e uartn, char *ch)
{
    //if( UART_RCFIFO_REG(UARTN[uartn]) )         //查询是否接受到数据
	if ((UART_S1_REG(UARTN[uartn]) & UART_S1_RDRF_MASK))
    {
        *ch  =   UART_D_REG(UARTN[uartn]);      //接受到8位的数据
 ...
 */
char uart_querystr (UARTn_e uartn, char *str, uint32 max_len)
{
    uint32 i = 0;
    while(uart_querychar(uartn, str + i)  )
    {
        if( *(str + i) == '\0' )    //接收到字符串结束符
        {
            return i;
        }
 ...
 */
char uart_querybuff (UARTn_e uartn, char *buff, uint32 max_len)
{
    uint32 i = 0;
    while(uart_querychar(uartn, buff + i)  )
    {
        i++;
        if(i >= max_len)            //超过设定的最大值，退出
        {
 ...
 */
void uart_putchar (UARTn_e uartn, char ch)
{
    //等待发送缓冲区空
...
{
    if(UART_RCFIFO_REG(UARTN[uartn]))                 //接收到数据数量非0
        //if(!(UART_SFIFO_REG(UARTN[uartn]) & UART_SFIFO_RXEMPT_MASK))
        //if(UART_S1_REG(UARTN[uartn]) & UART_S1_TDRE_MASK)
    {
        return 1;
    }
    else
    ...
    //return UART_RCFIFO_REG(UARTN[uartn]);
}
 ...
 */
void uart_putstr (UARTn_e uartn, const char *str)
{
    while(*str)
    {
...
{
    UART_C2_REG(UARTN[uartn]) |= UART_C2_RIE_MASK;                          //使能UART接收中断
    enable_irq((IRQn_t)((uartn << 1) + UART0_RX_TX_IRQn));                  //使能IRQ中断
}
...
{
    UART_C2_REG(UARTN[uartn]) |= UART_C2_TIE_MASK;                          //使能UART发送中断
    enable_irq((IRQn_t)((uartn << 1) + UART0_RX_TX_IRQn));                  //使能IRQ中断
}
 ...
 *  Sample usage:       uart_tx_irq_en(UART3);         //开串口3发送中断
 */
void uart_txc_irq_en(UARTn_e uartn)
{
    UART_C2_REG(UARTN[uartn]) |= UART_C2_TCIE_MASK;                         //使能UART发送中断
    enable_irq((IRQn_t)((uartn << 1) + UART0_RX_TX_IRQn));                  //使能IRQ中断
}
...
{
    UART_C2_REG(UARTN[uartn]) &= ~UART_C2_RIE_MASK;                         //禁止UART接收中断

    //如果发送中断还没有关，则不关闭IRQ
    if(!(UART_C2_REG(UARTN[uartn]) & (UART_C2_TIE_MASK | UART_C2_TCIE_MASK)) )
    {
        disable_irq((IRQn_t)((uartn << 1) + UART0_RX_TX_IRQn));             //关IRQ中断
    }
}
 ...
 */
void uart_txc_irq_dis(UARTn_e uartn)
{
    UART_C2_REG(UARTN[uartn]) &= ~UART_C2_TCIE_MASK;                        //禁止UART发送完成中断

    //如果接收中断还没有关，则不关闭IRQ
    if(!(UART_C2_REG(UARTN[uartn]) & UART_C2_RIE_MASK) )
    {
        disable_irq((IRQn_t)((uartn << 1) + UART0_RX_TX_IRQn));             //关IRQ中断
    }
}
...
{
    UART_C2_REG(UARTN[uartn]) &= ~UART_C2_TIE_MASK;                         //禁止UART发送中断
