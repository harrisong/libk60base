 ...
 *  @param	  flag		 	Flag to indicate whether MOSI/MISO wire is connected
 *  @since      v5.0
...
uint32 spi_init(SPIn_e spin, SPIn_PCSn_e pcs, SPI_CFG master, uint8_t flag, uint32 baud)
{
	...
	if(spin == SPI0)
	{
		...
		if (flag & SPI_MOSI_FLAG)
			port_init(SPI0_SOUT, ALT2  );
		if (flag & SPI_MISO_FLAG)
...
SPI_CLK_EXIT:
        ...
        SPI_CTAR_REG(SPIN[spin], 0) = (0
                                       ...
                                       //| SPI_CTAR_CSSCK(1)    //
                                       | SPI_CTAR_PCSSCK(0x00)    //设置片选信号有效到时钟第一个边沿出现的延时的预分频值。tcsc延时预分频 2*x+1；
                                      );
    }
    else
    {
