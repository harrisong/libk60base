...
uint32 i2c_init(I2Cn_e i2cn, uint32 baud)
{
    if(i2cn == I2C0)
    {
        ...
        /* 配置 I2C0功能的 GPIO 接口 */
        if(I2C0_SCL == PTB0)
            port_init (PTB0, ALT2 | PULLUP );
        else if(I2C0_SCL == PTB2)
            port_init (PTB2, ALT2 | PULLUP );
        else if(I2C0_SCL == PTD8)
            port_init (PTD8, ALT2 | PULLUP );
        else
            ASSERT(0);                              //上诉条件都不满足，直接断言失败了，设置管脚有误？

        if(I2C0_SDA == PTB1)
            port_init (PTB1, ALT2 | PULLUP );
        else if(I2C0_SDA == PTB3)
            port_init (PTB3, ALT2 | PULLUP );
        else if(I2C0_SDA == PTD9)
            port_init (PTD9, ALT2 | PULLUP );
        else
            ASSERT(0);                              //上诉条件都不满足，直接断言失败了，设置管脚有误？
    }
    else
    {
        ...
        /* 配置 I2C1功能的 GPIO 接口 */
        if(I2C1_SCL == PTE1)
            port_init (PTE1, ALT6 | PULLUP );
        else if(I2C1_SCL == PTC10)
            port_init (PTC10, ALT2 | PULLUP );
        else
            ASSERT(0);                          //上诉条件都不满足，直接断言失败了，设置管脚有误？

        if(I2C1_SDA == PTE0)
            port_init (PTE0, ALT6 | PULLUP );
        else if (I2C1_SDA == PTC11)
            port_init (PTC11, ALT2 | PULLUP );
        else
            ASSERT(0);                          //上诉条件都不满足，直接断言失败了，设置管脚有误？
    }
