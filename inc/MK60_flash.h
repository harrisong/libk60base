...
__RAMFUNC    uint8_t   flash_erase_sector  (uint16 sectorNo);                                                      //擦除指定flash扇区

__RAMFUNC    uint8_t   flash_write         (uint16 sectorNo, uint16 offset, FLASH_WRITE_TYPE data);                          //写入flash操作
__RAMFUNC    uint8_t   flash_write_buf     (uint16 sectorNo, uint16 offset, uint16 cnt, uint8_t buf[]);              //从缓存区写入flash操作
